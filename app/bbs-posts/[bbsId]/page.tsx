...
async function getBBSPost(id: number) {
    const response = await fetch(`http://localhost:3000/api/post/${id}`, {
        cache: "no-store",
    })

    const bbsPost: BBSPost = await response.json();
    return bbsPost;
}
const BBSDetailPage = async ({ params }: { params: { bbsId: number } }) => {
    const bbsPost = await getBBSPost(params.bbsId)
    const { title, content, username } = bbsPost;
    return (
